...
from SA_Attempt3_C2 import SA_FullMulti
import SAProcessing_C as SAP
...
problem = {
    'num_vars': 7,
    'names': ['TotDepth', 'widthRS', 'lengthTotal', 'vflow','G', 'T_K', 'SPM'],
             # 'alpha','t_half_d',,,'density','shape','sizeBin','t_frag_d','t_biof_growth_d'
             # ,'discharge','length_a','diameter'],
             # , ,'length_b','length_c'],
...
Si_Var = SAP.SA_indices(problem,Si,'Output_Mean',method,'SA_ResultsCC','No_Notes',N_samples)
#Placed at end so that files not unnecessarily generated for failed tests
sys_in = SAP.SAin(problem,param_values,'SA_InputsCC')

Ti = sobol.analyze(problem, E.values)
Ti_Var = SAP.SA_indices(problem,Ti,'Output_Mean',method,'SA_ResultsCC','No_Notes',N_samples)
sys_in = SAP.SAin(problem,param_values,'SA_InputsCC')

Ui = sobol.analyze(problem, F.values)
Ui_Var = SAP.SA_indices(problem,Ui,'Output_Mean',method,'SA_ResultsCC','No_Notes',N_samples)
sys_in = SAP.SAin(problem,param_values,'SA_InputsCC')

Vi = sobol.analyze(problem, G.values)
Vi_Var = SAP.SA_indices(problem,Vi,'Output_Mean',method,'SA_ResultsCC','No_Notes',N_samples)
sys_in = SAP.SAin(problem,param_values,'SA_InputsCC')
