...
from SA_Attempt3_A3 import SA_FullMulti
import SAProcessing_A as SAP
...
N_samples = 4096 #180 #2358 #393 for FAST
...
problem = {
    'num_vars': 3,
    'names': ['density','shape','sizeBin'],
             # 'alpha','t_half_d',,,,'t_frag_d','t_biof_growth_d''TotDepth', 'widthRS', 'lengthTotal', 'vflow',
             # ,'discharge','length_a','diameter''G', 'T_K', 'SPM'],
             # , ,'length_b','length_c'],
...
Si_Var = SAP.SA_indices(problem,Si,'Output_Mean',method,'SA_ResultsAAA','No_Notes',N_samples)
#Placed at end so that files not unnecessarily generated for failed tests
sys_in = SAP.SAin(problem,param_values,'SA_InputsAAA')

Ti = sobol.analyze(problem, E.values)
Ti_Var = SAP.SA_indices(problem,Ti,'Output_Mean',method,'SA_ResultsAAA','No_Notes',N_samples)
sys_in = SAP.SAin(problem,param_values,'SA_InputsAAA')

Ui = sobol.analyze(problem, F.values)
Ui_Var = SAP.SA_indices(problem,Ui,'Output_Mean',method,'SA_ResultsAAA','No_Notes',N_samples)
sys_in = SAP.SAin(problem,param_values,'SA_InputsAAA')

Vi = sobol.analyze(problem, G.values)
Vi_Var = SAP.SA_indices(problem,Vi,'Output_Mean',method,'SA_ResultsAAA','No_Notes',N_samples)
sys_in = SAP.SAin(problem,param_values,'SA_InputsAAA')
